...
- 5492d1a: Add `zora agent create` and `zora agent coin` for fully autonomous Zora agent onboarding.
...
- 0513708: Add a global, wallet-level spending budget for agents that caps total spend across all trading skills, on top of each skill's own caps.
...
- 7f0946b: Record the full agent identity in the wallet file after `zora agent create`.
...
- 6d180b1: Add `zora agent connect-email` to link an email to an existing Zora agent account.
...
- c94fd90: make the buy command smart wallet aware (support buying with smart wallet through user operations)
- 3899426: Add support for smart wallet accounts and bundler clients
- 380a8f3: add profile identifier support to the send command (specify the recipient via 0x... address or Zora profile handle)
- 613f83f: Make the balance command smart wallet aware (if a smart wallet has been configured via env var or wallet.json, balance now returns that smart wallet's balance).
- 46d7970: make the sell command smart wallet aware (support selling with smart wallet through user operations)
- 95d08d1: Add an `onboarding` skill to `zora skills`.
...
- 3ee617b: make the send command smart wallet aware (support sending with smart wallet through user operations)
- 5feffe1: add a smart wallet top up suggestion to user operations which fail due to insufficient gas balance
- aa8a133: Add `zora skills` subcommand to install agent skills into the local agent's skills directory.
...
- d460453: Add ten new agent skills to `zora skills` and group every skill by category.
...
- febc6e2: add create command to the cli, allowing users to create posts

### Patch Changes

- e8bb124: Refresh the bundled agent SKILL.md to document the agent-capable CLI: creating a full on-chain identity (Zora profile, Coinbase Smart Wallet, creator coin, and first post), trading, market research, smart-wallet behavior, and direct messages.
- b6d99ac: Always report the profile and first-post links after `zora agent create` finishes. The first-post link previously depended on resolving the content-coin address from the inline `submitUserOperation` logs, which are routinely empty under headless/CI runs — so the post link was almost never shown. The address is now also resolved from the mined transaction's receipt, and falls back to the agent's profile URL when it still can't be pinned down, so a link is always available.
...
- bdc49b4: Guard destructive commands against silently wrecking an agent setup
...
- 310741a: Add a `--ticker` flag to `zora agent create`'s first post and enforce a title length limit.
...
- 47b79d5: Include the wallet address in `zora balance --json`, `zora balance spendable --json`, and `zora balance coins --json` output.
...
- 9c4a3d8: Authenticate `zora dm` with the cached/refreshing Privy session.
...
- 56c0e14: Reuse Privy sessions instead of re-running Sign-In-With-Ethereum for every agent operation.
...
- 6b34a84: Show the smart wallet address in `zora wallet info`.
...
- 6fcbf9c: Fix the XMTP native-binding libiconv patch failing to apply inside Nix shells
...
- c1b09ed: fix static gas reserves in buy and send commands for smart wallet user operations
- 5caad2f: Fix the first-post step of `zora agent create` failing with a server validation error ("expected array, received undefined"). The content-coin creation request must send the agent's admin addresses under the Zora backend's `adminAddressess` field; restore that exact key so the first post is published successfully.
- 34f0339: ensure the bundler client uses realistic gas fee estimations to prevent excessive overestimation of gas fees and insufficient gas failures when executing user operations
- 978816e: Support a fully non-interactive `agent connect-email` flow. Running with `--json` (or `--yes`) and no `--code` now sends the one-time code and exits with `codeSent: true` instead of opening an interactive prompt. Re-run with `--email <email> --code <code> --json` to finish linking, so an agent can drive the flow while the operator relays the emailed code.
- Updated dependencies [c8374a6]
- Updated dependencies [6638205]
- Updated dependencies [1121bfe]
- Updated dependencies [e1fa9e7]
